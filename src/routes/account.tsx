...
function Account() {
  ...
  useEffect(() => { if (!loading && !user) window.location.href = "/auth"; }, [user, loading]);
  ...
  useEffect(() => {
    ...
    supabase.from("customisation_requests").select("*").eq("user_id", user.id).order("created_at", { ascending: false })
  ...
  return (
    <PageShell eyebrow="Your atelier" title="My account." intro={user.email ?? ""}>
      <div className="mb-10 flex flex-wrap gap-3">
        {user?.email === "hello@ethima.in" && <Link to="/admin" className="border border-champagne/60 px-5 py-3 text-[0.65rem] tracking-[0.28em] uppercase text-champagne hover:bg-champagne hover:text-primary-foreground transition-all">Admin Dashboard</Link>}
        <button onClick={() => signOut().then(() => window.location.href = "/")} className="border border-border px-5 py-3 text-[0.65rem] tracking-[0.28em] uppercase text-ivory/80 hover:border-champagne hover:text-champagne transition-all">Sign out</button>
      </div>
      <div className="mb-14">
        <h2 className="eyebrow mb-5 text-champagne">My orders</h2>
        {orders.length === 0 ? <p className="text-sm text-ivory/55">No orders yet.</p> : (
          <ul className="space-y-3">
            {orders.map((o) => (
              <li key={o.id} className="border border-border p-5 bg-[oklch(0.16_0.025_200/0.6)]">
                <div className="flex flex-wrap justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-ivory/40">{new Date(o.created_at).toLocaleDateString("en-IN")}</p>
                  </div>
                  <span className="self-start border border-border px-3 py-1.5 text-[0.6rem] tracking-[0.2em] uppercase text-champagne">{o.status}</span>
                </div>
              </li>
            ))}
          </ul>
