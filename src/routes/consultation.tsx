...
import { useEffect, useState, type ReactNode, type ComponentType, type FormEvent } from "react";
...
export const Route = createFileRoute("/consultation")({
  head: () => ({
    meta: [
      { title: "Book Your Consultation — ethima | Personalised Jewellery Design" },
      { name: "description", content: "Book a one-to-one consultation with the ethima atelier. Personal guidance, bespoke design support, no obligation. Let's create something meaningful together." },
      { name: "keywords", content: "jewellery consultation india, custom jewellery design, bespoke jewellery consultation, ethima consultation, personal jewellery design" },
      { property: "og:title", content: "Book Your Consultation — ethima" },
      { property: "og:description", content: "Personal guidance to design a piece that's truly yours." },
      { name: "robots", content: "index, follow" },
    ],
    links: [{ rel: "canonical", href: "https://ethima.in/consultation" }],
  }),
  component: Consultation,
});

const TYPES = ["Bespoke Design", "Engagement Ring", "Personalisation", "General Guidance"];
const MODES = ["Video Call", "WhatsApp"];
...
function Consultation() {
  ...
  useEffect(() => {
    if (!loading && !user) window.location.href = "/auth?redirect=/consultation";
  }, [user, loading]);
